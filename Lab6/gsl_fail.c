...
double f_der(double x, void* param) {
    if(x>1) return 1.0;
    return (x-2)*x + 1;
}
double df_der(double x, void* param) {
    if(x>1) return 0.0;
    return 2*x-2;
}

void fdf_der(double x, void* param, double* f, double* df) {
    *f =  f_der (x,NULL);
    *df = df_der (x,NULL);
}
...
int derivatives( double x0, double x, const gsl_root_fdfsolver_type *T) {
  int iter = 0, status;
...
int main (void) {
	...
	struct quadratic_params params = {1.0, -6.0, 9.0};
	
	double x_lo = 0, x_hi = 5.0;
  ...
  int status;
	...
	switch(c) {
		...
		case '3':
			...
			break;
		case '4':
			status = derivatives(x_lo, x_hi, gsl_root_fdfsolver_newton);
			break;
		case '5':
			status = derivatives(x_lo, x_hi, gsl_root_fdfsolver_secant);
			break;
		case '6':
			status = derivatives(x_lo, x_hi, gsl_root_fdfsolver_steffenson);
			break;
		default:
